...
module.exports = class wordpressClass {
    constructor(wordpressSetup) {
        ...
        this.slugIds = wordpressSetup.slugIds;
        this.slugs = [];
        this.tagSlugs = [];
        this.categorySlugs = [];
    ...
    async getPosts() {
        const posts = [];
        this.slugs = Object.keys(this.slugIds);
        for (const slug of this.slugs){
            const slugPosts = [];
            const url = `${this.restUrl}${slug}?include[]=` + this.slugIds[slug].join('&include[]=');
            const data = await this.customAxios(url);
